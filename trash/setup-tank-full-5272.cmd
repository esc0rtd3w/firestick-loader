...
:: Get FireOS Version
for /f "tokens=3 delims= " %%a in ('type "%temp%\fireos-version.txt"') do set fireOsVersion=%%a
...
:stage1
:: Set default FW Version
set fwVersion=5.2.6.3
color 0e
set rwcheck=0
cls
echo.
...
if %fwVersion%==0.0.0.0 (
cls
echo Current FW Version [%fwVersion%]
echo.
echo Set FW Version to 5.2.7.2 [Y/N]?
echo.
echo.
set /p fwVersionSelect=
:forcechk
if %fwVersionSelect%==N set fwVersion=5.2.6.3
if %fwVersionSelect%==n set fwVersion=5.2.6.3
if %fwVersionSelect%==Y set fwVersion=5.2.7.2
if %fwVersionSelect%==y set fwVersion=5.2.7.2
if %fwVersionSelect%==0 goto forcechk
)

::echo fwVersionSelect: %fwVersionSelect%
...
:: If 5.2.7.2 skip debloat until after OOBE is completed (for now!)
if %fwVersion%==5.2.7.2 goto setup

if %fwVersion%==5.2.7.2-forced (
echo.
...
if %fwVersion%==5.2.6.3 goto setup

:: FireOS 5.2.6.7
:: Alexa Media Controller
%shell% "rm -r /system/app/com.amazon.alexa.externalmediaplayer.fireos/"
...
if %fwVersion%==5.2.7.2-forced set fwVersion=5.2.7.2
if %fwVersion%==5.2.7.2 goto final

:setup
if %fwVersion%==5.2.7.2 set fwVersion=5.2.7.2-forced
%sleep% 5
...
if %fwVersion%==5.2.7.2 goto skipoobe
if %fwVersion%==5.2.7.2-forced goto skipoobe
...
echo.
echo Turn on USB Debugging under Settings - My FireTV - Developer Options
echo.
pause>nul
%adbWait%
%adb% reboot recovery
...
if %fwVersion%==5.2.7.2 %push% %equipment% /data/local/tmp/
...
if %fwVersion%==5.2.7.2 %shell% "cp /data/local/tmp/equipment.sh /system/scripts/equipment.sh"
...
if %fwVersion%==5.2.7.2 goto final5272
if %fwVersion%==5.2.7.2-forced goto fix5272
...
pause>nul
if %fwVersion%==5.2.7.2-forced goto debloat
goto end
