...
:: Used for 5.0.5 when no downgrade will be done
set nodg=0
...
echo.
%cocolor% 0e
echo Do you want to downgrade to 5.0.5 [Y/N]?
echo.
echo.
set /p dgchoice=
echo.
...
%msgbox% "You must have already completed the initial OOBE setup on device to continue!" "Montoya Full Setup Script"
...
:stage1
if %downgrade%==1 (
	set accessibility="scripts\settings\montoya\system\scripts\505\accessibility.sh"
	set applications="scripts\settings\montoya\system\scripts\505\applications.sh"
	set btcontroller="scripts\settings\montoya\system\scripts\505\btcontroller.sh"
	set device="scripts\settings\montoya\system\scripts\505\device.sh"
	...
	set notifications="scripts\settings\montoya\system\scripts\505\notifications.sh"
	set preferences="scripts\settings\montoya\system\scripts\505\preferences.sh"
)

if %downgrade%==0 (
	set accessibility="scripts\settings\montoya\system\scripts\5272\accessibility.sh"
	set alexa="scripts\settings\montoya\system\scripts\5272\alexa.sh"
	set applications="scripts\settings\montoya\system\scripts\5272\applications.sh"
	set btcontroller="scripts\settings\montoya\system\scripts\5272\btcontroller.sh"
	set device="scripts\settings\montoya\system\scripts\5272\device.sh"
	set displaysounds="scripts\settings\montoya\system\scripts\5272\display-sounds.sh"
	set equipment="scripts\settings\montoya\system\scripts\5272\equipment.sh"
	set help="scripts\settings\montoya\system\scripts\5272\help.sh"
	set myaccount="scripts\settings\montoya\system\scripts\5272\my-account.sh"
	set network="scripts\settings\montoya\system\scripts\5272\network.sh"
	set notifications="scripts\settings\montoya\system\scripts\5272\notifications.sh"
	set preferences="scripts\settings\montoya\system\scripts\5272\preferences.sh"
)
color 0e
set rwcheck=0
...
:: If version is 5.0.5 then do not downgrade
...
cls
echo Preparing Stock FireOS 5.0.5 Downgrade Files
echo.
if not exist "%temp%\firestick-loader\downgrade\stick\5.0.5" md "%temp%\firestick-loader\downgrade\stick\5.0.5"
%extractRAR% "downgrade\stick\5.0.5\firmware-505.split" "%temp%\firestick-loader\downgrade\stick"

cls
echo Pushing Downgrade Bin to /sdcard/...
echo.
%push% "%temp%\firestick-loader\downgrade\stick\update-kindle-montoya-54.5.3.7_user_537174420.bin" /sdcard/
%sleep% 2
...
cls
echo Installing Stock FireOS 5.0.5...
echo.
%twrp% install /sdcard/update-kindle-montoya-54.5.3.7_user_537174420.bin
%sleep% 5

cls
echo Cleaning Up Files...
echo.
rd /s /q "%temp%\firestick-loader\downgrade\stick"
%shell% "rm /sdcard/update-kindle-montoya-54.5.3.7_user_537174420.bin"
%sleep% 2
...
if %downgrade%==1 %push% "data\montoya\post-debloated\505\restore" /sdcard/restore/
if %downgrade%==0 %push% "data\montoya\post-debloated\5272\restore" /sdcard/restore/
%sleep% 2
...
::cls
::echo Re-Installing SuperSU for SU Access...
::echo.
::%push% "rooting\%supersuZip%" /data/local/tmp/
::%twrp% install /data/local/tmp/%supersuZip%
::%sleep% 2
